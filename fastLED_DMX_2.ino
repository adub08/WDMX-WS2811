...
//  EEPROM.write(1, 64);
  ...
  ArduinoDmx0.set_rx_address(EEPROM.read(0));      // set rx0 dmx start address
  ...
  FastLED.addLeds<WS2811, DATA_PIN, COLOUR_ORDER>(leds, NUM_LEDS); //initialize LED Pixel output. Using WS2811 12 volt Pixels.
  attachInterrupt(digitalPinToInterrupt(disconnectPin), disconect, LOW);
  FastLED.setMaxPowerInVoltsAndMilliamps(5,2000);  //#experiemental# sets max power at 25w for LEDS
...
channel +=channelwidth; //increase last DMX channel number by channel width
  ...
  delay(60); //delay 60 miliseconds for DMX to catch up.  Less delay will cause flicker. 
} 
