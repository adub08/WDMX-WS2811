//Written by Alex Woodmansey using Version 1.6.8 - 19-06-2016
...
#define NUM_LEDS 11 //Number of LED Pixels connected
...
const int WDMX_button = A5;     // Internal WDMX Button Pin
...
void setup() 
{ 
//  EEPROM.write(0, 1);   //Alternative addressing method by holding the DMX address in the EEPROM 
//  EEPROM.write(1, 64);
  ...
  FastLED.addLeds<WS2811, DATA_PIN, COLOUR_ORDER>(leds, NUM_LEDS); //initialize LED Pixel output
  attachInterrupt(digitalPinToInterrupt(disconnectPin), disconect, LOW);
  FastLED.setMaxPowerInVoltsAndMilliamps(12,1250);  //#experiemental# sets max power at 13.5w for LEDS
  

  post(); //run power on self test function each time board resets 
  SYS_TEST(); //Checks Battery voltage and displays over pixels
}  //end setup()
 ...
 void SYS_TEST() //Will show battery voltage on a scale of 0-1023 over 0-NUM_LEDs [requires calabration]
{
  ...
  fill_solid( leds, NUM_LEDS, CRGB(255,255,255)); // All LEDs Full at white 
  FastLED.show();
  delay(300);
...
void post() //power on self test function.  Make sure all pixels are working and run through colors.
{
 ...
 void disconect() //Disconnects WDMX module, currently runs within ISR - could be nicer. 
{
